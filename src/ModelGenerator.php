...
class ModelGenerator
{
    ...
    /**
     * Get the preferred display column for a related table.
     *
     * @param string $table
     *
     * @return string
    ...
    public function getFirstColumn($table)
    {
        ...
        foreach ($tableColumns as $column) {
            if ($firstFallbackColumn === '' && $column->Field !== 'id') {
                $firstFallbackColumn = $column->Field;
            }
            ...
            if (
                ...
                $firstReadableColumn = $column->Field;
                break;
            }
        }

        if ($firstReadableColumn !== '') {
            return $firstReadableColumn;
        }

        if ($firstFallbackColumn !== '') {
            return $firstFallbackColumn;
        }

        return $tableColumns[0]->Field ?? 'id';
    }
    ...
    private function _getFunction(string $relation, string $table, string $foreign_key, string $local_key)
    {
        ...
        if ($relation === 'belongsTo') {
            $relationCall = "\$this->{$relation}('{$this->modelNamespace}\\\\{$model}', '{$local_key}', '{$foreign_key}')";
        } else {
            $relationCall = "\$this->{$relation}('{$this->modelNamespace}\\\\{$model}', '{$foreign_key}', '{$local_key}')";
        }
    ...
    ';
        ...
        if ($relation == 'hasOne' || $relation == 'belongsTo') {
            $return .= '
    public function '.$relationName.'SelectAll()
    {
        return  (new \\'.$this->modelNamespace.'\\'.$model.')->pluck(\''.$this->getFirstColumn($table).'\',\''.$foreign_key.'\')->toArray();
    }
    ';
        }
